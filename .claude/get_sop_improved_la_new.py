...
def _parallel_match_worker(task: Tuple[int, str]) -> Tuple[int, Optional[Dict[str, Any]], Dict[str, Any], Optional[str]]:
    """
    ...
    if isinstance(potential_utterances, dict):
        ...
        for item_name in identified_items:
            ...
            if selected_value is not None:
                storage_key = item_name[4:] if isinstance(item_name, str) and item_name.startswith("客户回复") else item_name
            ...
            if tasks:
                ...
                try:
                    with ProcessPoolExecutor(
                        ...
                        initargs=(sop_data, similarity_threshold, sentencewise_params["strong_validation_default_threshold"], sentencewise_params["strong_validation_erci_threshold"])
                    ) as executor:
                        future_to_idx = {
                            executor.submit(_parallel_match_worker, task): task[0]
                            for task in tasks
                        }
                        for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="并行匹配", leave=False):
                            try:
        ...
        for idx, (_, row) in enumerate(tqdm(rows, total=len(rows), desc="分析对话记录")):
            ...
            if sentencewise_triggered:
                ...
                if sentencewise_debug.get("max_similarity_reference"):
                    fallback_notes.append(f"句级备用最高相似度参考话术: {sentencewise_debug['max_similarity_reference']}")
            ...
            if sentencewise_match and fallback_path_list:
                ...
                if 2 in fallback_levels:
                    fallback_level2_sim = fallback_levels[2].get('similarity', sentencewise_match.get('similarity', 0.0))
                else:
            ...
            if sentencewise_triggered:
                if primary_match and primary_match.get('strong_validation_used'):
                    ...
                    if fallback_level2_name and fallback_level2_sim >= sentencewise_params["override_level2_similarity"] and fallback_level2_name != primary_level2:
                        selected_match = _extend_sentencewise_branch(sentencewise_match, conversations, sop_data, sentencewise_params)
            ...
            if sentencewise_match:
                ...
                fallback_trace_json = json.dumps(fallback_trace, ensure_ascii=False) if fallback_trace else fallback_trace_json
            ...
            strong_level2_similarity = selected_match.get('strong_validation_level2_similarity', 0.0) if selected_match else 0.0
            ...
            strong_sales_message = selected_match.get('strong_validation_level2_sales_message', '') if selected_match else ''
            ...
            strong_best_path = " -> ".join(match_debug.get('strong_validation_best_level2_path', [])) if match_debug else ''
            strong_coverage_ratio = selected_match.get('strong_validation_coverage_ratio', 0.0) if selected_match else 0.0
            ...
            if isinstance(regular_tail_expected, dict):
                ...
                regular_tail_expected = json.dumps(regular_tail_expected, ensure_ascii=False) if regular_tail_expected else ''
            ...
            labeled_record = {
                ...
                "强校验匹配路径": strong_path,
                "强校验预期SOP路径": selected_match.get('strong_validation_expected_path', '') if selected_match else '',
