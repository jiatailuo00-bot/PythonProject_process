...
CONFIG_PATH = BACKEND_DIR / "config/zlkt_matcher.json"
DATA_DIR = Path(__file__).resolve().parent.parent / "data"

_DEFAULT_CONFIG = {
    "default_excel_path": "badcase2233.xlsx",
    "default_sop_json": "logic_tree_zlkt_origin2.json",
    "history_fallbacks": ["历史对话", "完整历史对话"],
    "default_mode": "both",
    "sales_keywords": "销售,CSM",
    "similarity_threshold": 0.75,
}


def _load_matcher_config() -> Dict[str, object]:
...
MATCHER_CONFIG = _load_matcher_config()
...
DEFAULT_EXCEL_PATH = _resolve_path(BACKEND_DIR.parent, MATCHER_CONFIG.get("default_excel_path", "badcase2233.xlsx"))
DEFAULT_SOP_JSON_PATH = _resolve_path(DATA_DIR, MATCHER_CONFIG.get("default_sop_json", "logic_tree_zlkt_origin2.json"))
DEFAULT_HISTORY_FALLBACKS = MATCHER_CONFIG.get("history_fallbacks", ["历史对话", "完整历史对话"])
DEFAULT_MODE = MATCHER_CONFIG.get("default_mode", "both")
DEFAULT_SALES_KEYWORDS = MATCHER_CONFIG.get("sales_keywords", "销售,CSM")
DEFAULT_SIMILARITY_THRESHOLD = float(MATCHER_CONFIG.get("similarity_threshold", 0.75))
VERIFY_COLUMN_MAP = {
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Minimum similarity score to accept a match (default: {DEFAULT_SIMILARITY_THRESHOLD}).",
    )
    parser.add_argument(
        "--sales-keywords",
        default=DEFAULT_SALES_KEYWORDS,
        help="Comma-separated keywords that denote sales-side speakers in历史对话.",
    )
    parser.add_argument(
        "--history-column",
        default=DEFAULT_HISTORY_FALLBACKS[0],
